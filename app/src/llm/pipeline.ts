...
const MARKUP_HINT =
  "排版约定（配合报告库「编辑体」样式，会渲染成对应模块）：核心动作是把连续文字拆进结构块——正文段落每段 2–4 句、只做过渡与串联；一个章节若通篇是大段落就是排版失败，务必把要点切成组件。" +
  "① 并列的多个要点 / 一段列举 → 用 ```what 要点块代替（每行：小标签 | 一句话；最重要的一项行首加 * 标为重点）。" +
  "② 多维对比 / 分类 → markdown 表格（合计 / 总计行首列用 **加粗**）；产业链 → ```chain；时间演进 → ```timeline；交易结构 → ```dealflow；一组可比数值（占比 / 增速 / 评分）→ ```mrow（每行：名称 | 百分比数字 | 显示值 | 语义色 teal/gold/red/blue）。" +
  "③ 结尾的综合研判用一个 ```verdict（每行：利好 / 需冷静 / 核心结论 | 一句话），正反与结论并陈。" +
  "④ 想强调的判断 / 风险 / 洞察，用「> 结论：…」「> 风险：…」「> 洞察：…」各自成段（适度、不滥用）。" +
  "⑤ 章节标题用「主题：一句概括」式（如「需求侧：谁在买、为什么买」），层次用 ## 章 / ### 小节。" +
  "围栏写法（重要）：开栏行只写 ``` 紧跟英文类型名本身、后面不要接任何标题或中文（对：单独一行 ```what；错：```what 典型收入模型）——要给某块加小标题，就在围栏前单独写一行 #### 小标题；每块内部字段一律用 | 分隔（与 chain 一致）。以上结构块按内容合适度选用，不为用而用。" +
  "禁止：不要在正文里成段罗列「需补资料：…」或「需补 / 未公开」清单——缺口只在相关句子里用一句话内嵌注明即可（如「（口径待核）」「（未公开）」），让成品读起来是一份定稿、而非工作笔记。";

// 有联网检索资料时的引用约定（无检索时这条无害）
const CITE_HINT =
  "若「资料研判」中含带编号 [n] 与链接的检索资料，引用其事实 / 数据时在句末标注对应 [n]；不要自行罗列参考文献，系统会在文末统一附上。";

// 篇幅要求：写足、写透（配合命中上限自动续写，篇幅不受单次限制）
const LENGTH_HINT =
  "篇幅要足、要深：每个要点都展开讲透，给足事实、数据、机制与案例，不要点到为止；目标是一份扎实完整的研究报告（正文约 8000–12000 字）。若一次没写完，可在中断处继续，系统会自动衔接。";
