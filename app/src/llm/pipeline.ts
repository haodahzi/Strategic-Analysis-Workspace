...
const MARKUP_HINT =
  "排版约定（配合报告库「编辑体」样式，会渲染成对应模块）：默认动作是把连续文字拆进结构块——段落每段 2–4 句、只做过渡，通篇大段文字＝排版失败。" +
  "① 开头给一个 ```kpi 关键数字快览，挑 3–4 个最关键的量化指标（每行：标签 | 数值 | 语义色 teal正面/gold重点/red风险/blue中性，可留空 | 一句说明）。" +
  "② 并列的多个要点、一段列举，用 ```what 要点块代替罗列（每行：小标签 | 一句话；最重要的一项行首加 * 标为重点），别堆成一大段。" +
  "③ 产业链用 ```chain、时间演进用 ```timeline、交易结构用 ```dealflow（写法见下方类型说明）。" +
  "④ 一组可比数值（占比 / 增速 / 评分）可用 ```mrow（每行：名称 | 百分比数字 | 显示值 | 语义色）。" +
  "⑤ 结尾的综合研判用一个 ```verdict（每行：利好 / 需冷静 / 核心结论 | 一句话），正反与结论并陈。" +
  "⑥ 想强调的判断 / 风险 / 洞察，可用「> 结论：…」「> 风险：…」「> 洞察：…」各自成段（适度、不滥用）；对比数据用 markdown 表格，合计 / 总计行首列用 **加粗**；层次用 ## / ### 小标题。" +
  "以上结构块统一用 ``` 围栏、字段用 | 分隔（和 chain 一致）；按内容合适度选用，不为用而用。";
