...
export const INDUSTRY_FRAME =
  "① 行业本质（这门生意靠什么创造价值、靠什么赚钱）｜" +
  "② 需求侧：谁在买、为何买、量在哪里（购买主体、需求驱动、市场规模与增速、需求结构）｜" +
  "③ 供给与竞争格局（主要玩家、集中度、进入壁垒）｜" +
  "④ 产业链与价值链（上中下游构成、各环节代表企业、利润沉在哪段、议价力如何分布）｜" +
  "⑤ 商业模式与盈利公式（典型收入＝量×价、成本结构、单位经济、规模 / 网络效应）｜" +
  "⑥ 技术与技术演进（关键技术与路线、成熟度、替代与迭代方向）｜" +
  "⑦ 政策与监管框架（相关政策方向、准入与合规约束）｜" +
  "⑧ 发展阶段与趋势（当前所处阶段、关键时间节点与阶段划分、增长驱动、未来走向与不确定性）";
...
const MARKUP_HINT =
  "排版：少写大段文字、多用结构，正文段落只做承接过渡、每段别超过 3 句；凡能拆成下面这些结构的，就别堆成大段落。" +
  "① 并列的要点 / 分类 / 「几个方面」→ 写成要点列表，每条用「- **要点标签**：一句话说清」的形式（系统会自动排成要点卡片，别写成一整段）。" +
  "② 多维对比 / 几种类型 / 前后对比 → markdown 表格（合计 / 总计行首列用 **加粗**）。" +
  "③ 产业链上中下游 → 一个 ```chain 代码块；时间 / 阶段演进 → ```timeline；一组可比数值（占比 / 增速 / 评分）→ ```mrow（每行：名称 | 百分比数字 | 显示值 | 语义色 teal/gold/red/blue）。" +
  "④ 重要结论 / 风险 / 洞察 → 用「> 结论：…」「> 风险：…」「> 洞察：…」各自单独成段（适度）；结尾的综合研判可用一个 ```verdict（每行：利好 / 需冷静 / 核心结论 | 一句话）。" +
  "⑤ 层次用 ## 章 / ### 小节，章节标题用「主题：一句概括」式（如「需求侧：谁在买、为什么买」）。" +
  "不要在正文里成段罗列「需补资料 / 需补 / 未公开」清单——缺口只在相关句子里用一句话内嵌注明即可（如「（口径待核）」「（未公开）」）。";
...
export function buildStageRequest(stage: PipelineStage, ctx: PipelineCtx, model: string): ChatRequest {
  ...
  switch (stage.id) {
    ...
    case "research":
      ...
      break;
    case "draft":
      user = `${head}\n内容骨架：\n${o.plan ?? ""}\n\n资料研判：\n${o.research ?? ""}\n\n据此起草正文，像一份严谨的研究综述：每部分先讲清事实与逻辑、再给有节制的判断；量化数据必带口径与来源，无来源标「需补 / 未公开」。紧扣上面框架各要点，不要出现框架名或教科书标签，不要清单感。用 markdown。${note} ${NEUTRALITY} ${MARKUP_HINT} ${CITE_HINT} ${LENGTH_HINT}`;
      break;
    case "red":
...
export function mockStageOutput(stage: PipelineStage, input: PipelineInput): StageResult {
  ...
  switch (stage.role) {
    case "规划":
      return { stageId: stage.id, summary: `按内置框架把${subj}（${input.industry}）定框：本质 → 需求（谁在买/为何买/量在哪里）→ 供给格局 → 产业链与价值链 → 商业模式 → 技术演进 → 政策监管 → 发展阶段与趋势，逐一落到具体情形；中性客观、不预设立场。` };
    case "资料":
