...
// 洽谈后·项目报告 = 可行性判断（可编辑#6）+ 交易结构（据实录入）+ 定调。
...
export default function ProjectReport({ analysis }: { analysis: Analysis }) {
  ...
  return (
    <div className="pr-report">
      ...
      <div className="set-hint" style={{ marginBottom: 10 }}>据本单在「调研前 · 深度分析」得到的研究与你补充的事实填写；空白起步，不套用模板。</div>
      ...
      <textarea className="nd-extra" value={tx} placeholder="据实录入这单的交易结构：各方出什么 / 拿什么、资金 / 货物 / 合同 / 发票怎么走…（洽谈后据实填，无则留空）" onChange={(e) => setTx(e.target.value)} />
    </div>
  );
}
