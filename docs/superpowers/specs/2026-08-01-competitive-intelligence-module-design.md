...
- 数据库按需惰性初始化。初始化失败不阻断整个工作台；用户进入模块或重试健康检查时重新尝试。
...
| `fetch_source_snapshot` | `{ request: { url, expectedHost } }` | 返回状态、最终 URL、类型、哈希、相对路径和时间；不返回正文 |
...
`fetch_source_snapshot` 只服务由模块配置的来源，并执行以下完整链路：

1. 解析 URL；仅允许 `https`，禁止用户名/密码，主机名必须与 `expectedHost` 不区分大小写地相等。
2. DNS 解析主机，拒绝任何回环、私网、链路本地、未指定、IPv6 unique-local 及云元数据地址；空解析也失败。
3. 将经校验的 IP 集合固定到本次 reqwest client/请求，连接不得再次使用未验证 DNS 结果，防止检查后使用（DNS rebinding）。
4. 禁用自动重定向；因此最终 URL 不会静默跳往未经验证的主机。如后续允许重定向，每一跳必须重复解析、校验并固定。
5. 设置 20 秒超时，只接受 HTML、XHTML、纯文本、XML、RSS 和 Atom 类型。
6. 若 `Content-Length` 已超过 5 MiB，读取前拒绝；无论该头是否存在，响应均按数据块流式读取，累计字节一旦超过 `5 * 1024 * 1024` 立即停止并拒绝。不得先调用 `bytes()` 后再检查长度。
7. 成功正文计算 SHA-256、gzip 写入内容寻址路径；IPC 只返回元数据。

测试覆盖 HTTP、主机不匹配、凭据 URL、IPv4/IPv6 私有地址、元数据地址、空 DNS、DNS 固定、禁重定向、错误 MIME、声明超限、无长度头的流式超限、边界内成功和快照去重。
...
- 浏览器模式：缺少 Tauri/keyring 不是致命错误；只禁用秘密持久化和 Rust 专属情报操作，并提供明确提示。

## 10. Capability 决策

`app/src-tauri/capabilities/default.json` 保持不变。现有 HTTP plugin 白名单只供既有模型提供商调用；情报来源请求由 Rust 内部 reqwest 命令执行，不依赖或扩大 WebView HTTP 权限。数据库、文件和凭据也均未通过插件 capability 暴露。若未来改为 WebView 直连或新增插件，必须另行安全评审，不能借本计划扩大权限。

## 11. 验收标准

- 既有 dashboard、新分析、报告库和设置正常；新增入口不会修改已有分析数据。
- 首次进入模块在 WebView 存储外创建独立 SQLite，迁移版本为 1；初始化失败后可重试且不拖垮应用。
- 启动恢复可标记合成的运行中记录并返回准确时间窗，同时证明没有发出采集请求。
...
- `Cargo.lock` 已跟踪并可由 Rust 1.77.2/MSVC 构建；Tauri capabilities 无 diff。
- 新模块无招聘 UI、表、采集器或文案。
